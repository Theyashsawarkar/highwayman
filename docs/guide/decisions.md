...
several attachments uploading at once; the replace only needs the
placeholder string to still be somewhere in the current value; a token
already scoped by only the current turn didn't need to be
...
### Verified against a disposable server + a hand-rolled fake runner, not the live one

**Why:** the live server/runner pair backing an active session is
somebody's actual working connection — restarting it to test against
would cut that off mid-work. Standing up a second server on a throwaway
port, plus a minimal WebSocket client reusing the real
`runner/src/actions.js#createActions` (so the binary/base64 write path
was genuinely exercised, not mocked), gave the same coverage — byte-exact
upload round-trips, path-traversal filenames, missing-file and
disconnected-runner errors — without touching anything already running.
